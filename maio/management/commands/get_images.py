...
class Command(MaioBaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        ...
        parser.add_argument(
            'directories', nargs='+', type=str, metavar='DIRECTORIES',
        ...
        # Tagging
        parser.add_argument(
            '--tag-directories', '-td', action='store_true',
            help=(
                'Tag the supplied directories for Image Tags. Does not tag '
                'subdirectories under the supplied directories.'
            )
        )

        parser.add_argument(
            '--tag-subfolders', '-ts', action='store_true',
            help=(
                'Tag the subdirectories under the supplied directories. Does '
                'not tag the supplied directories.'
            )
        )

        parser.add_argument(
            '--tag-filenames', '-tf', action='store_true',
            help=('Tag the file names of the files.')
        )

        parser.add_argument(
            '--tag-all', '-ta', action='store_true',
            help=('Equivalent to options -td -ts -tf.')
        )

        parser.add_argument(
            '--tags', '-t', nargs='*', type=str, metavar='TAG',
            help=('Tag each image with %(metavar)s.')
        )

        # Meta
        parser.add_argument(
            '--love', '-lv', action='store_true',
        ...
        parser.add_argument(
            '--rating', '-r', type=int, metavar='RATING',
            help=('Rate each image with %(metavar)s. Choices are integers from 0 to 5, inclusive.')
        )

        parser.add_argument(
            '--author', '-a', type=str, metavar='AUTHOR',
            help=('Set the %(metavar)s for each image.')
        )

        parser.add_argument(
            '--url', '-u', type=str, metavar='URL',
            help=('Set the %(metavar)s for each image.')
        )

        parser.add_argument(
            '--source', '-s', type=str, metavar='SOURCE',
            help=('Set the %(metavar)s for each image.')
        )

        parser.add_argument(
            '--copyright', '-cr', type=str, metavar='COPYRIGHT',
            help=('Set the %(metavar)s for each image.')
        )

        parser.add_argument(
            '--comment', '-co', type=str, metavar='COMMENT',
            help=('Set the %(metavar)s for each image.')
        )

        parser.add_argument(
            '--set-inactive', '-si', action='store_true',
            help=(
                'By default, all images are set to an active state. '
                'Use this flag to set each image as inactive.'
            )
        )

        parser.add_argument(
            '--set-hidden', '-sh', action='store_true',
            help=(
                'By default, all images are not hidden. '
                'Use this flag to set each image as hidden.'
            )
        )

        parser.add_argument(
            '--set-deleted', '-sd', action='store_true',
            help=(
                'By default, all images are not delete. '
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        def is_image(mimetype: str) -> bool:
            ...
            '''
            for key, _value in lib.MIMETYPE_EXTENSION['image'].items():
                if mimetype == key:
                    return True
            return False
        ...
        # tag flag options
        tag_directories = options.get('tag_directories')
        tag_subfolders = options.get('tag_subfolders')
        tag_filenames = options.get('tag_filenames')
        tag_all = options.get('tag_all')
        tags_input = options.get('tags')
        is_loved = options.get('love')
        is_liked = options.get('like')
        ...
        set_inactive = options.get('set_inactive')
        set_hidden = options.get('set_hidden')
        set_deleted = options.get('set_deleted')
        ...
        for directory in directories:
            ...
            for root, _subdirs, files in os.walk(directory):
                ...
                for filename in files:
                    ...
                    # get mime type
                    try:
                        mimetype = mime.from_file(file_path) # type: ignore[reportUnknownMemberType]
                    except IOError as e:
                    ...
                    # get file extension
                    filename_ext: tuple[str, ...] = lib.MIMETYPE_EXTENSION['image'].get(
                        mimetype,
                        tuple(tuple())
                    )[0]
